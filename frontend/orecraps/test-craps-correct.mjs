#!/usr/bin/env node
/**
 * Test Craps game with correct devnet mint address
 */
import { 
  Connection, 
  PublicKey, 
...
async function main() {
  ...
  // Check craps game exists
  const gameAccount = await connection.getAccountInfo(gameAddress);
  if (!gameAccount) {
    console.log("Craps game does NOT exist!");
    return;
  }
  console.log("Craps game exists! Data length:", gameAccount.data.length);
  ...
  try {
    ...
    console.log("No CRAP balance found - need to get some CRAP tokens first");
  }
  
  // Check vault balance
  try {
    const balance = await connection.getTokenAccountBalance(vaultAta);
    console.log("Vault CRAP balance:", balance.value.uiAmount);
  } catch (e) {
    console.log("No Vault balance found");
  }
  
  // Try to add more funds to craps house
